...
async def video_whiteboard_plan(req: VideoWhiteboardPlanReq,
    ...
    sys = (
        ...
        "'stomach', 'spoon', 'clock', 'rocket') — ALWAYS English even when labels are another "
        "language; TRANSLATE Indonesian subjects to English (kereta→train, rel→railway, sungai→river, "
        "sawah→rice field, gunung→mountain, kapal→ship, pasar→market, kebun teh→tea plantation). Pick "
        "the CONCRETE noun, not an abstract phrase (for 'kereta tua meluncur' use 'train', not 'old "
        "train sliding'). It looks up an icon library; no file names, no phrases; "
