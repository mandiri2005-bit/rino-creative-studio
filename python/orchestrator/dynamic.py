...
_STORY_BIBLE_SYSTEM_FICTION = (
    ...
    "closed, when an incident happened) as ONE fixed month/year AND ONE fixed reason/mechanism — "
    "not just the story's own overall elapsed span — so a later chapter cannot independently give "
    "the same past event a different date or a different cause. When a sequence of steps leading "
    ...
    "left off-screen. Note, per thread, roughly which late chapter delivers its outcome. Two "
    ...
    "earlier chapter both times, not two different answers.\n"
    ...
    "for one the premise left anonymous, transplanting one entry's detail onto another entry, or "
    "swapping which entry belongs to which year. For a list-type exhibit with more than roughly 25 "
    ...
    "roster. If an exhibit CHANGES HANDS over the course of the story, pin the exact custody "
    ...
    "\"chain of hands\" instead). Skip this heading only if the premise has no shared physical "
    "exhibits referenced "
    "across chapters.\n"
...
_STORY_BIBLE_SYSTEM_NONFICTION = (
    ...
    "business closed, when an incident happened) as ONE fixed date AND ONE fixed reason from the "
    "premise/topic only, or [VERIFY] if not given — never let a later chapter independently state "
    "a different date or cause for the same event. When a sequence of steps leading to an event is "
    ...
    "only, or [VERIFY] — rather than letting it surface for the first time at the climax.\n"
    ...
    "entry's detail onto another or swap which entry belongs to which year. For a list-type "
    ...
    "its provenance, rather than the full roster. If an exhibit CHANGES HANDS, pin the exact "
    ...
    "independently invent a different giver, location, or link count for the same handoff.\n"
