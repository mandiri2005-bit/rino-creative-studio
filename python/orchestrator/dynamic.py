...
# ===========================================================================
# FICTION variant — the outline owns the story; the bible only freezes compatible specifics.
_STORY_BIBLE_SYSTEM_FICTION = (
    ...
    "contradict each other. The accepted CHAPTER OUTLINE is the immutable, highest-authority "
    "story specification. Preserve every explicit or clearly implied event, causal relation, "
    "motivation, identity, relationship, reveal, resolution, chapter assignment, and chronology "
    "exactly as outlined. This bible is a derived continuity sheet, never an alternative draft. "
    "You may fix a detail only when the outline genuinely leaves that detail unspecified and the "
    "choice does not add, remove, move, or reinterpret a plot beat. If a requested field would "
    "conflict with the outline, keep the outline's version and leave the extra detail unspecified.\n\n"
    ...
    "One person, one FIXED set of facts, in every chapter. This includes WHO currently cares for a "
    "minor or dependent character (a parent, a grandparent, another guardian) — fix this once; a "
    "later chapter must not silently swap the caregiver (a present, named parent in one chapter "
    "becoming an unexplained grandparent in another) without an on-page account of what happened "
    "to the original guardian. If a character's AGE and a separate CAREER/TENURE DURATION are both "
    ...
    "one name, one spelling. This also covers NAMED INSTITUTIONS the story treats as a location or "
    "venue (a specific police precinct, a prosecutors' office, a court, a district office) — pin "
    "the exact name and jurisdiction once; a later chapter must not reference the same institution "
    "under a different name or in a different district.\n"
    ...
    "consistent while preserving the outline's chronology exactly, including deliberate "
    "flashbacks, parallel tracks, or time-skips. Lay out a CHAPTER TIMELINE that records each "
    "chapter's outlined time position; do not reorder or flatten it into a forward sequence. "
    "Within each chapter, preserve the written order of its outlined sentences and beats unless "
    "the outline explicitly marks a flashback or parallel track; a later threat, reveal, decision, "
    "or resolution may not be moved ahead of an earlier beat. If the premise, title, contract, "
    "countdown, or ending establishes a bounded duration or deadline, pin a minimal visible "
    "story-clock marker for EVERY chapter (for example day/week position or elapsed interval) so "
    "the final expiry cannot arrive through an unaccounted-for time jump. Fill only time positions "
    "the outline leaves open; never move or add an event to make the clock work. If a "
    "chapter is pinned to a "
    "specific month or season, any atmospheric or seasonal descriptive detail in that chapter "
    "(weather, light, season-specific imagery) must match that pinned month — a later-drafted "
    "chapter must not use a seasonal descriptor (e.g. \"December sky\") that contradicts its own "
    "pinned date. PIN EVERY EVENT-SPECIFIC DATE "
    "more than one chapter will reference (when a character was fired, when a business or plant "
    "closed, when an incident happened) as ONE fixed month/year, ONE fixed reason/mechanism, AND "
    "ONE fixed LOCATION — not just the story's own overall elapsed span — so a later chapter "
    "cannot independently give the same past event a different date, a different cause, or a "
    "different place. Where the story's own vagueness could read as DELIBERATELY leaving open "
    "whether two such backstory events happened at the SAME location or at different ones, COMMIT "
    "to one answer now — state plainly whether they share a location or not — rather than letting "
    "that ambiguity survive into chapters that each independently assume their own answer. When a "
    "sequence of steps leading "
    "to an event is given explicit day-level timing (e.g. \"filed on Tuesday, badge revoked "
    "Wednesday, fired Friday\"), pin the EXACT elapsed duration those steps imply (here: 3 days "
    "from filing to firing) as its own fixed fact — a later chapter must not restate the same "
    "sequence with a different total duration (e.g. \"six days\"). This also covers any OTHER "
    "precisely-pinned quantitative fact that changes over the story's timeline (a subscriber "
    "count, a follower count, a financial figure): if the story states an exact number as fixed "
    "at one point (\"four hundred and six, I keep the number exact\"), a later chapter's larger "
    "or smaller number must be explicitly derived (state the trigger and the elapsed time), not "
    "simply asserted as a new fact disconnected from the earlier pinned exact number. If a "
    "population COUNT changes "
    "because the story kills, removes, relocates, or adds to it (deaths, departures, new arrivals, "
    "people falling ill), do the arithmetic ONCE, right here, and state the resulting number as "
    "the one fixed count every later chapter must use — never leave a later chapter to re-derive "
    "its own subtraction or addition from the same inputs. Be explicit about WHICH category each "
    "number is: a headline total the story states (\"84 residents\") already means what it says at "
    "that point in the timeline — do not treat it as a gross figure still owing a further "
    "deduction unless the story itself says so, and do not conflate a count of people CURRENTLY "
    "PRESENT with a count of people historically part of the same population but no longer present "
    "(already dead, already relocated) — these answer different questions, and a later chapter "
    "must not silently subtract one from the other a second time. If the story ALSO names specific "
    "individuals within a population (e.g. three named sick children), the headline count and the "
    "number of named individuals must be reconcilable — a later chapter must not introduce a "
    "bigger number (\"nine sick children\") with no basis anywhere in what this sheet actually "
    "names. When the named individuals are vulnerable (sick, injured, or otherwise harmed "
    "children or dependents), decide HERE, as a general staging POLICY (not a per-scene "
    "schedule — this sheet has no visibility into individual scenes within a chapter), a rough "
    "ceiling on how many of them need to be physically on the page in any chapter/scene that "
    "features them versus referenced, spoken of, or represented by a smaller on-page subset — do "
    "not require every chapter that mentions the group to physically parade the full named set "
    "through the scene.\n"
    "5. POV & NARRATION — who narrates and whose head we are in (per chapter if it rotates), "
    "first vs third person, and tense. Fix this so no chapter silently switches viewpoint. This "
    "applies to the FINAL chapter too: if every other chapter is past tense, the last chapter "
    "must also be past tense — unless the outline itself calls for a marked epilogue, in which "
    "case name it as such (an \"Epilogue\" heading or equivalent) and commit to the shift "
    "deliberately, not as a silent, unmarked change a reader would read as a mistake.\n"
    "6. KEY FACTS / REVEAL — the plot facts that must not drift (who did what to whom, the "
    "secret, the diagnosis, the twist). Pin the SINGLE version specified by the outline and state "
    "it once. If the outline leaves a mystery's solution unspecified, mark it UNSPECIFIED IN "
    "OUTLINE rather than inventing the real identity, what happened, or the kinship/blood facts. "
    "For an outlined solution, make every clue, document, "
    ...
    "committed answer; they are never improvised against it. This also covers any ESTABLISHED "
    "CONSTRAINT OR RULE the plot sets up (a legal injunction's exact scope, a stated \"if X "
    "happens, Y consequence\" rule) — pin exactly what the constraint covers, once, so no later "
    "chapter's action or narration silently violates or contradicts it (e.g. if a chapter "
    "establishes that publishing a specific figure counts as contempt of court, a later chapter "
    "must not describe a character having done exactly that without acknowledging the "
    "violation).\n"
    ...
    "Record the payoff, unresolved ending, and owning chapter exactly as the outline assigns them; "
    "never manufacture a payoff or move one to a different chapter. Four "
    "more requirements belong here. First, if the premise's evidence chain depends on an "
    "anonymous informant, tipster, or whistleblower, preserve whether the outline makes that "
    "figure a single individual who stays anonymous or is later named. Only if that identity "
    "question is genuinely unspecified and has no effect on an outlined reveal may you pin one "
    "compatible answer. If the outline's chapter "
    "summaries would otherwise let separate chapters each introduce what reads as its own "
    "anonymous source (a leaker, a caller, a tipster), collapse them into that ONE person unless "
    "the premise genuinely requires multiple distinct informants — do not let chapter-by-chapter "
    "drafting accumulate an uncounted crowd of anonymous sources. If this figure's unmasking IS "
    "the #8 SIGNATURE HOOK, heading 8's committed payoff (who they are and which chapter reveals "
    "it) governs this too — do not pin a second, conflicting answer here. Second, the single "
    "piece of evidence, testimony, or document that decides the story's climactic hearing, "
    "confrontation, or verdict must retain the planting chapter the outline gives it; never move "
    "or add a planting scene. If the outline supplies no earlier plant, mark that gap UNSPECIFIED "
    "IN OUTLINE instead of repairing it here. If NARASI_CONTINUITY_PINS' heading 18 EVIDENCE "
    "CHAIN & CUSTODY is "
    "also active for this book, this is the SAME requirement, not a second one — give the same "
    "earlier chapter both times, not two different answers. Third, if the premise involves a "
    "missing, captive, kidnapped, or imprisoned character whose fate is eventually resolved (found, "
    "rescued, escapes, or is released), pin the outlined circumstance, timing, location, mechanism, "
    "and owning chapter. You may fill an omitted mechanism only when the outline already requires "
    "that same resolution in that same chapter and the detail changes no beat, so two chapters "
    "cannot each independently invent their own incompatible account of how or when that character "
    "was found or released. Fourth, IF the premise establishes a piece of physical evidence as "
    "tainted, unlawfully obtained, or inadmissible (a stolen sample, an illegally recorded tape, "
    "evidence a character gloats cannot be used in court), pin how the outline resolves the "
    "lawful-evidence question and which chapter owns that resolution. If the outline gives no "
    "answer, mark it UNSPECIFIED IN OUTLINE; do not insert an official sample, denial, plea, "
    "settlement, or other new resolution. This requirement is dormant for stories with no "
    "tainted-evidence setup.\n"
    ...
    "answers it.\n"
    "21. EXHIBITS — every documentary or physical evidence object more than one chapter will "
    "describe (a signature, a permit, a core/soil sample, a bank record, a recording, a "
    "photograph): pin its ONE physical or damage state, its exact dimensions/quantity, AND the "
    "named person or organization attached to it (who signed it, who paid or received a transfer, "
    "who it belongs to) now, so two chapters written in parallel do not each invent an incompatible "
    "version of the same object or its parties (a signature cannot be both cut out with a blade in "
    "one chapter and present-and-forensically-compared in another; a core cannot be both a "
    "3.8-meter borehole in one chapter and an unexplained 40-centimeter tube in the next; a bank "
    "transfer's payer cannot be one company in one chapter and a different company in another). "
    "This also covers a LIST-TYPE exhibit that itself enumerates named sub-records (a death "
    "ledger, a witness roster, an evidence log, a manifest of names and dates): pin EACH entry's "
    "name, date, and defining detail as its own fixed fact, once, in full — a later chapter that "
    "reads the list aloud must reproduce the SAME entries verbatim, never inventing a full name "
    "for one the premise left anonymous, transplanting one entry's detail onto another entry, "
    "swapping which entry belongs to which year, or swapping the ENTIRE enumerated set for a "
    "different, non-identical set (a different total count, new unlisted entries, dropped "
    "previously-pinned entries). For a list-type exhibit with more than roughly 25 "
    "entries, pin only the entries the outline's own chapter summaries already name or that carry "
    "a turning point (at most 25), plus the total count and its provenance, rather than the full "
    "roster. NEAR-DUPLICATE EXHIBITS: when the story contains two or more SIMILAR documents or "
    "exhibits of the same kind (two internal memos, two ledgers, two contracts), pin EACH one "
    "separately with its OWN distinguishing attributes — its own date, its own distinctive "
    "label/terminology, its own provenance and chain-of-custody (who surfaced it, whether it is "
    "authenticated or an anonymous copy) — and NEVER let a later chapter re-describing one "
    "document blend the other's attributes into it: borrowing the other document's date, quoting "
    "the other's column label or terminology, or attributing the other's provenance is a canon "
    "fork, not a paraphrase. If an exhibit CHANGES HANDS over the course of the story, pin the exact custody "
    "SEQUENCE now — who received it from whom, at what location, at what point in the timeline, "
    "for every link in the chain — as one fixed sequence; a later chapter must not independently "
    "invent a different giver, a different location, or a different link count for the same "
    "handoff (e.g. one chapter cannot have the owner personally hand over an object at a described "
    "meeting while a later chapter has them absent and the object arriving through an unspecified "
    "\"chain of hands\" instead). This custody sequence is about a physical OBJECT changing hands — "
    "it is DISTINCT from a separate pin-type below for who legally OWNS the story's central asset. "
    "If the story's central asset itself has a multi-entity OWNERSHIP CHAIN or CHAIN OF TITLE — who "
    "legally owns or owned it, across what sequence of named entities (a shell company, a holding "
    "company, an individual), with what transfer dates — pin that ownership chain as its OWN fixed "
    "sequence now, once: name each entity in order, the exact date each transfer of ownership took "
    "effect, and which entity holds title at the story's present moment. A later chapter must not "
    "independently invent a different owner, insert or drop an entity in the sequence, or restate a "
    "different transfer date for the same ownership chain. Skip the exhibit-object portion of this "
    "heading only if the premise has no shared physical exhibits referenced across chapters; skip "
    "the ownership-chain portion only if no asset in the story changes legal ownership.\n"
    "22. PACING & TIME-SKIPS — if the ending shows a multi-step technical or administrative "
    "process COMPLETING (an environmental remediation, a construction project, a bureaucratic "
    "reform, a large-scale medical recovery, an arrest followed by a formal indictment) that "
    "realistically takes weeks or months — site control, procurement, contractor mobilization, "
    "funding authorization, healing time, a prosecutor's charging review — the "
    "timeline must respect that: plan an explicit time-skip (\"Three months later\") between the "
    "immediate/emergency response (a halt, a fence, monitoring, an arrest) and the process being "
    "shown as complete (remediation finished, reform enacted, recovery achieved, indictment "
    "handed down), rather than compressing a multi-month real-world process into the space of "
    "days with no skip marked. An arrest is not the same event as an indictment: if the story "
    "shows both, pin them as two separate dated beats with a realistic gap, not a single action a "
    "chapter narrates as though the two were simultaneous. This must agree with heading 4 and the "
    "outline. If the outline itself fixes a shorter interval, record that interval unchanged; do "
    "not silently repair, widen, or relocate it in the bible. Skip this heading only if the ending "
    "shows no such process completing.\n\n"
    "Rules: extract and consolidate the outline first. Fix only genuinely unspecified continuity "
    "details that cannot alter its plot. Never override, correct, improve, or redistribute the "
    "outline. Do NOT write prose, plot beats, or chapter content. Keep it tight — a numbered "
...
_STORY_BIBLE_SYSTEM_NONFICTION = (
    ...
    "piece is about. The accepted CHAPTER OUTLINE is immutable and outranks this derived sheet: "
    "preserve every explicit or clearly implied claim, event, chapter assignment, and ordering; "
    "never rewrite the outline to satisfy a generic continuity preference.\n\n"
    ...
    "roles; keep them consistent (no renaming, no invented middle names). This includes WHO the "
    "premise/topic states currently cares for a minor or dependent (a parent, a grandparent, "
    "another guardian) from the premise/topic only, or [VERIFY] if not given — never let a later "
    "chapter silently swap in a different caregiver without that being what the source material "
    "says. If the premise/topic gives BOTH a person's age AND a separate career/tenure duration "
    "(e.g. an age and years in a prior role), confirm the arithmetic given is internally possible "
    "and cross-check it against any other biographical date pinned elsewhere in this sheet — flag "
    "\"[VERIFY: age/tenure arithmetic]\" rather than silently repeating a contradiction if the "
    "source material itself does not resolve it.\n"
    "2. CENTRAL SUBJECT — the one thing the piece is about; its GIVEN attributes only.\n"
    "3. SETTING — named real places, spelled consistently; do not invent their backstory. This "
    "also covers NAMED INSTITUTIONS the piece treats as a location or venue (a specific police "
    "precinct, a prosecutors' office, a court, a district office) — pin the exact name and "
    "jurisdiction from the premise/topic only, or [VERIFY] — never let a later chapter reference "
    "the same institution under a different name or district.\n"
    "4. TIMELINE — the ordering/era the premise establishes; unknown dates → [VERIFY]. If a chapter "
    "is pinned to a specific month or season (established from the premise/topic only, or "
    "[VERIFY] if not given), any atmospheric or seasonal descriptive detail in that chapter must "
    "match that pinned month — a later chapter must not use a seasonal descriptor contradicting "
    "its own pinned date. PIN EVERY "
    "EVENT-SPECIFIC DATE more than one chapter will reference (when a person was fired, when a "
    "business closed, when an incident happened) as ONE fixed date, ONE fixed reason, AND ONE fixed "
    "LOCATION from the premise/topic only, or [VERIFY] if not given — never let a later chapter "
    "independently state a different date, cause, or place for the same event. Where the source "
    "material's own vagueness could read as leaving open whether two such backstory events happened "
    "at the SAME location or at different ones, note the single best-supported answer from the "
    "premise/topic only, or write \"[VERIFY: shared location?]\" — never let that ambiguity survive "
    "into chapters that each independently assume their own answer. When a sequence of steps leading to an event is "
    "given explicit day-level timing in the source material (e.g. filed on one day, a following "
    "action the next, a resolution some days later), pin the exact elapsed duration those steps "
    "imply as its own fixed fact from the premise/topic only, or [VERIFY] — never let a later "
    "chapter restate the same sequence with a different total duration. This also covers any OTHER "
    "precisely-given quantitative fact that changes over the timeline (a subscriber count, a "
    "follower count, a financial figure): if the premise/topic states an exact number as fixed at "
    "one point, a later chapter's different number must be explicitly derived (state the trigger "
    "and elapsed time) from the premise/topic only, or [VERIFY] — never asserted as a new fact "
    "disconnected from the earlier pinned number. If a population COUNT changes because the "
    "premise's subject loses or gains members (deaths, departures, arrivals), state the resulting "
    "number ONCE from the premise/topic only, or [VERIFY] — never let a later chapter re-derive a "
    "different arithmetic from the same inputs. Be explicit about WHICH category a number is: a "
    "headline total already means what it says at that point in the timeline — do not treat it as "
    "a gross figure owing further deduction unless the premise says so, and do not conflate people "
    "CURRENTLY PRESENT with people historically part of the same population but no longer present "
    "— these answer different questions. If the premise ALSO names specific individuals within a "
    "population, the headline count and the number of named individuals must be reconcilable — a "
    "later chapter must not introduce a bigger number with no basis in what this sheet names. When "
    "the named individuals are vulnerable (sick, injured, or otherwise harmed children or "
    "dependents), note from the premise/topic only, as a rough staging ceiling, how many actually "
    "need to appear physically in any chapter/scene that features them versus being referenced — "
    "do not require every chapter that mentions the group "
    "to stage the full named set on the page.\n"
    "5. POV & NARRATION — the narrating stance and tense to hold throughout. This applies to the "
    "final chapter too — a silent tense shift in the last chapter (past tense elsewhere, present "
    "tense there) is a defect, not a stylistic choice, unless the premise explicitly calls for a "
    "marked epilogue that commits to the shift deliberately.\n"
    "6. KEY CLAIMS — the through-line the premise sets; anything not given → [VERIFY], not invented. "
    "This also covers any ESTABLISHED CONSTRAINT OR RULE the premise sets up (a legal injunction's "
    "exact scope, a stated \"if X happens, Y consequence\" rule): pin exactly what it covers, from "
    "the premise/topic only, or [VERIFY] — never let a later chapter's narration silently violate "
    "or contradict it without acknowledging the violation. If the premise's evidence chain depends "
    "on an anonymous informant, tipster, or whistleblower, note from the premise/topic only whether "
    "that is one figure or several, and keep the count consistent across chapters rather than "
    "letting separate chapters each imply their own uncounted anonymous source. If the premise "
    "identifies the evidence, testimony, or document that decides a climactic hearing or "
    "confrontation, note which earlier chapter it should already appear in — from the premise/topic "
    "only, or [VERIFY] — rather than letting it surface for the first time at the climax. If the "
    "premise/topic involves a missing, captive, or imprisoned person whose fate is resolved (found, "
    "rescued, or released), note from the premise/topic only the single circumstance and timing of "
    "how and when that resolution happens, and which chapter it appears in — or write \"[VERIFY: "
    "resolution circumstance/timing]\" — never let two chapters independently invent their own "
    "incompatible account of how or when that person was found or released. If the premise/topic "
    "establishes a piece of physical evidence as tainted, unlawfully obtained, or inadmissible, "
    "note from the premise/topic only how the lawful-evidence question resolves once full legal "
    "standing is reached (an arrest, a hearing, probable cause) — an official sample/inspection "
    "obtained, sought and denied or delayed, or rendered moot (a plea, a settlement, remediation "
    "under supervision) — and which chapter addresses it, or write \"[VERIFY: lawful-evidence "
    "resolution]\" — never let the question silently vanish from the chapters once raised.\n"
    "7. EXHIBITS — every documentary or physical evidence object more than one chapter will "
    "describe (a signature, a permit, a core/soil sample, a bank record, a recording): pin its "
    "ONE physical or damage state, its exact dimensions/quantity, and the named person or "
    "organization attached to it (who signed it, who paid or received a transfer, who it belongs "
    "to) from the premise/topic only, or write \"[VERIFY: ...]\" — never invent it, and never let "
    "two chapters written in parallel independently describe the same object or its parties "
    "incompatibly. This also covers a LIST-TYPE exhibit that itself enumerates named sub-records "
    "(a death ledger, a witness roster, an evidence log): pin EACH entry's name, date, and "
    "defining detail from the premise/topic only (or \"[VERIFY: ...]\" per entry) — never invent a "
    "full name for one the premise left anonymous, and never let a later chapter transplant one "
    "entry's detail onto another, swap which entry belongs to which year, or swap the ENTIRE "
    "enumerated set for a different, non-identical set (a different total count, new unlisted "
    "entries, dropped previously-pinned entries). For a list-type "
    "exhibit with more than roughly 25 entries, pin only the entries the outline's own chapter "
    "summaries already name or that carry a turning point (at most 25), plus the total count and "
    "its provenance, rather than the full roster. NEAR-DUPLICATE EXHIBITS: when the piece "
    "involves two or more SIMILAR documents of the same kind (two internal memos, two ledgers, "
    "two contracts), pin EACH one separately with its OWN date, its own distinctive "
    "label/terminology, and its own provenance/chain-of-custody — each attribute from the "
    "premise/topic only, or \"[VERIFY: ...]\" per attribute — and never let a later chapter blend "
    "one document's date or quoted terminology into a description of the other. If an exhibit "
    "CHANGES HANDS, pin the exact "
    "custody SEQUENCE (who received it from whom, at what location, when, for every link) from "
    "the premise/topic only, or write \"[VERIFY: ...]\" per link — never let a later chapter "
    "independently invent a different giver, location, or link count for the same handoff. This "
    "custody sequence is about a physical OBJECT changing hands — it is DISTINCT from a separate "
    "pin-type below for who legally OWNS the piece's central asset. If the central asset itself has "
    "a multi-entity OWNERSHIP CHAIN or CHAIN OF TITLE — who legally owns or owned it, across what "
    "sequence of named entities (a shell company, a holding company, an individual), with what "
    "transfer dates — pin that ownership chain from the premise/topic only, as its OWN fixed "
    "sequence distinct from the custody sequence above (or write \"[VERIFY: ownership chain]\" per "
    "link) — never invent an entity, a transfer date, or which entity currently holds title, and "
    "never let a later chapter independently restate a different owner or transfer date for the "
    "same ownership chain.\n"
    "8. PACING & TIME-SKIPS — if the ending shows a multi-step technical or administrative "
    "process COMPLETING (a remediation, a construction project, a reform, a large-scale recovery, "
    "an arrest followed by a formal indictment) "
    "that realistically takes weeks or months, plan an explicit time-skip between the immediate "
    "response (including an arrest, if the premise has one) and the process being shown as "
    "complete (including an indictment, if the premise has one), from the premise/topic only, or "
    "[VERIFY] "
    "the realistic duration — do not compress a multi-month real-world process into days with no "
    "skip marked. An arrest and an indictment are not the same event — if the premise/topic gives "
    "both, pin them as two separately dated beats, not one action, or [VERIFY] the gap. This "
    "must agree with heading 4 and the accepted outline. If the source fixes a shorter interval, "
    "record it unchanged and mark the realism concern [VERIFY] rather than widening or relocating "
    "an outlined event.\n\n"
    "Rules: consistency, not invention. The outline always wins over this sheet. Do NOT write "
    "prose. Numbered sheet, one item per line, no preamble."
)
...
def _story_bible_prompt(topic: str, outline: list[dict], language: str, is_fiction: bool,
    ...
    heads = ("CHARACTERS, CENTRAL SUBJECT, SETTING, TIMELINE & QUANTITIES, POV & NARRATION, "
             "KEY FACTS / REVEAL, OPEN THREADS TO RESOLVE, SIGNATURE HOOK & ITS PAYOFF, EXHIBITS, "
             "PACING & TIME-SKIPS"
             if is_fiction else
             "SUBJECT / PEOPLE, CENTRAL SUBJECT, SETTING, TIMELINE, POV & NARRATION, KEY CLAIMS, "
             "EXHIBITS, PACING & TIME-SKIPS")
...
async def build_story_bible(
    ...
    *,
    ...
    if is_fiction and os.environ.get("NARASI_CANON_REGISTRY", "0").strip().lower() in ("1", "true", "yes", "on"):
        # FIX (2026-07-18, event-cap root-cause): scale the events cap by chapter count instead of
        # a flat 8 — a longer/more-conspiratorial book needs more registry slots. Same formula as
        # the diff-loop's own re-slice in narration_api.py's _v3g_canon_diff_detect, so the bible's
        ...
        system = system + (
            ...
            "\"anchor_chapter\":<n>,\"since_event\":<bool, optional>}]}. RULES: pin ONLY facts a chapter's plot turns on — an "
            "event enters ONLY if it carries a moral_load; hard caps <=" + str(_ncr_event_cap) + " "
            "events — if the story has MORE load-bearing events than this cap, TRIAGE: favor an "
            "event referenced by 3+ later chapters, or one already carrying a false_versions entry, "
            "over a lower-stakes event when deciding which make the cut — plus <=12 "
            "timeline anchors, <=10 kinship pairs, <=6 exhibit_sets, <=6 chains, <=8 quantities, so you TRIAGE, not dump. "
            "exhibit_sets is for a LIST-TYPE document/exhibit packet (heading 21's enumerated set) — "
            "one row per set, with its complete entry list; NEAR-DUPLICATE documents (heading 21's "
            "near-duplicate exhibits, e.g. two similar internal memos) each get their OWN "
            "exhibit_sets entry — never merged into one — with each document's distinguishing date "
            "and label pinned in its own entry. chains is for a multi-entity OWNERSHIP/"
            "CUSTODY TRANSFER GENEALOGY (heading 21's ownership chain / chain of title) — do NOT put "
            "these in kinship, which is for person-to-person family relations only, never a "
            "corporate transfer sequence. quantities is for a standalone precisely-pinned duration/"
            "count not tied to one event (a tenure, an age, a total) — anchor_chapter is where it is "
            "first pinned. Use canonical TOKENS/ints (victim=entity_id, "
            "age=int), NEVER prose descriptors (so 'dusk' vs 'evening' cannot fork). Set "
            "since_event:true when the quantity is a duration/tenure/age measured FORWARD FROM A "
            "FIXED PAST POINT (may legitimately grow — or, in a flashback chapter narrating an "
            "earlier point in the story's own chronology, legitimately be smaller — as the story's "
            "internal clock moves; e.g. a tenure, an age, an elapsed span since an incident) rather "
            "than a flat count with no time dimension (a bag total, a headcount); leave it unset for "
            "flat counts. If the story "
    ...
    if (is_fiction
        ...
        system = system + (
            ...
            "to narrate the story in order, is NOT a violation of this rule; likewise, a "
            "deliberate flashback or memory sequence placed LATER in the book that narrates an "
            "EARLIER point in the story's own chronology (before the event) is also NOT a "
            "violation — only an unintentional, unlabeled contradiction is. Omit both "
