...
_STORY_BIBLE_SYSTEM_FICTION = (
    ...
    "the story must not step backward between consecutive chapters. If a chapter is pinned to a "
    ...
    "left off-screen. Note, per thread, roughly which late chapter delivers its outcome. Four "
    "more requirements belong here. First, if the premise's evidence chain depends on an "
    "anonymous informant, tipster, or whistleblower, DECIDE NOW whether that figure is a single "
    "individual who stays anonymous throughout or is later named, and if the outline's chapter "
    ...
    "piece of "
    "evidence, testimony, or document that decides the story's climactic hearing, confrontation, "
    "or verdict must be PLANTED on the page in an earlier chapter (named, described, or directly "
    "foreshadowed there) — never introduced for the first time at the climax itself; note which "
    "earlier chapter plants it. If NARASI_CONTINUITY_PINS' heading 18 EVIDENCE CHAIN & CUSTODY is "
    ...
    "rescued, escapes, or is released), DECIDE NOW the single circumstance and timing of how and "
    "when that resolution happens — the location, the mechanism (rescued by whom, escapes how, "
    "released under what condition), and which chapter delivers it — once, here, so two chapters "
    ...
    "evidence a character gloats cannot be used in court), then once the story reaches full legal "
    "standing — an arrest, a hearing win, probable cause established — DECIDE NOW how the "
    "lawful-evidence question resolves and which chapter addresses it ON THE PAGE: the authorities "
    "obtain their own official sample/inspection, OR one is sought and denied or delayed, OR the "
    "question is rendered moot (a plea, a settlement, remediation under supervision) — the "
    "manuscript must address it at least once rather than silently dropping the question the "
    "moment the plot no longer needs it. This requirement is dormant for stories with no "
    ...
    "chapter narrates as though the two were simultaneous. This OVERRIDES heading 4's FORWARD "
    "CHAPTER TIMELINE where the two "
    "conflict: if the outline places the emergency-response chapter and the completion chapter on "
    "adjacent or same-day dates, widen the gap between those two chapters' pinned dates here — do "
    "not leave heading 4's chapter-order dates unchanged and only add this clause as unenforced "
    "prose. Skip this heading only if the ending shows no such process completing.\n\n"
...
_STORY_BIBLE_SYSTEM_NONFICTION = (
    ...
    "OVERRIDES heading 4's ordering where the two conflict: if the premise "
    "places the response and the completion too close together, widen the gap here rather than "
    "leaving heading 4's dates unchanged.\n\n"
