...
# ── Fix 4: cross-container cancel via Redis ──────────────────────────────────
# cancel_flags holds in-process threading.Events (needed for thread signalling).
# We ALSO mirror cancel state to Redis so a cancel request that lands on a
# different container than the running job is still seen. Loop checks consult
# both: the local Event (fast) and Redis (cross-container).
import redis.asyncio as _aioredis
_redis = _aioredis.from_url(os.getenv("REDIS_URL", "redis://localhost:6379"),
                            decode_responses=True)

async def request_cancel(key: str) -> bool:
    """Mark a job cancelled locally (if present) and in Redis. Returns True if a
    local Event existed."""
    await _redis.set(f"cancel:{key}", "1", ex=3600)
    flag = cancel_flags.get(key)
    if flag:
        flag.set()
        return True
    return False
...
async def is_cancelled(key: str) -> bool:
    """True if cancelled locally OR in Redis (covers cross-container cancels)."""
    flag = cancel_flags.get(key)
    if flag and flag.is_set():
        return True
    try:
        return await _redis.exists(f"cancel:{key}") == 1
    except Exception:
        return False  # Redis down → fall back to local-only, never block work

async def clear_cancel(key: str):
    cancel_flags.pop(key, None)
    try:
        await _redis.delete(f"cancel:{key}")
    except Exception:
        pass
...
async def cancel_stream(session_id: str):
    had_local = await request_cancel(session_id)
    if had_local:
        return {"status": "cancel_requested", "session_id": session_id}
    # No local Event: job may be running on another container — Redis flag is set,
    # the running loop will see it on its next check.
    return {"status": "cancel_requested_remote", "session_id": session_id}
...
async def _resolve_user_uuid(tenant_id: str, clerk_user_id: str) -> str:
    ...
    Looks up by external_id. Falls back to a deterministic UUID if not found.
    """
    try:
        row = await db._db().fetchrow(
            "SELECT id FROM users WHERE tenant_id=$1 AND external_id=$2",
            db._uid(tenant_id), clerk_user_id
        )
        if row:
            return str(row["id"])
    except Exception:
        pass
    # Fallback: derive deterministic UUID from clerk_user_id
...
async def stream_chat(req: ChatRequest,
    ...
    # fresh cancel flag for this request (in-memory — NOT migrated to DB)
    cancel_event = threading.Event()
    cancel_flags[_session_id] = cancel_event
    ...
    async def generate():
        ...
        try:
            ...
            cancel_flags.pop(_session_id, None)
            try:
                await clear_cancel(_session_id)
            except Exception:
                pass
            if not cancelled and chunks:
                ...
                try:
                    ...
                    print(f"[stream_chat] DB append error: {db_err}", flush=True)
        yield "data: [DONE]\n\n"
...
async def narasi_generate(body: dict):
    ...
    # Register cancel flag so client can call /narasi/cancel/{job_id}
    cancel_ev = threading.Event()
    cancel_flags[f"narasi_{job_id}"] = cancel_ev

    auto_cancelled = False
    bab1_words = None
    previous_chapters = []   # accumulate generated text for inter-chapter context
    for i, chapter in enumerate(chapters):
        # Check cancel before each chapter (local Event OR cross-container Redis flag)
        if cancel_ev.is_set() or await is_cancelled(f"narasi_{job_id}"):
        ...
        try:
            ...
            try:
                ...
                _moat_sid = await db.save_moat_session(
                    _ctx.tenant_id or db._DEV_TENANT_ID,
                    _ctx.user_id or None,
    ...
    # Clean up cancel flag (local + Redis)
    await clear_cancel(f"narasi_{job_id}")
    cancelled = cancel_ev.is_set()
    return {"ok": True, "job_id": job_id, "errors": errors, "cancelled": cancelled, "auto_cancelled": auto_cancelled,
...
async def narasi_cancel(job_id: str):
    ...
    key = f"narasi_{job_id}"
    had_local = await request_cancel(key)
    if had_local:
        return {"ok": True, "status": "cancel_requested", "job_id": job_id}
    return {"ok": True, "status": "cancel_requested_remote", "job_id": job_id}
...
async def oneshot_fix_submit(body: dict,
    ...
    def run_job():
        ...
        try:
            loop.run_until_complete(db.update_job_progress(job_id, "AI membaca seluruh manuskrip..."))
            resolved = MODELS.get(model, model)
            ceiling  = MODEL_MAX_TOKENS.get(resolved, DEFAULT_MAX_TOKENS)
            ...
            # Phase 1 migration: persist result to PostgreSQL
            loop.run_until_complete(db.complete_job(job_id, {
            ...
            loop.run_until_complete(db.fail_job(job_id, str(e)))
        finally:
            loop.close()
