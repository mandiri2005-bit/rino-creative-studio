...
- FORENSIC / SCIENTIFIC RESULT CLAIMS: when a scene uses a lab result, isotope match, or forensic test to PROVE a source-attribution claim (whose blood, whose remains, which site material came from), name the actual comparison in-scene (compared against what reference sample, by what marker) — a bare assertion that a test "confirmed" or "proved" an attribution with no stated mechanism reads as invented science; without a stated mechanism, downgrade the result to PROBABLE language ("consistent with", "matched to") rather than CERTAIN ("proved", "confirmed").
- EVIDENCE ARCHIVES used AS EVIDENCE (a company's own internally-kept files, a personal collection of records, an off-the-books log being relied on to prove a claim — not an ordinary keepsake or photo album with no evidentiary role in the scene) default to PROBABLE, not CERTAIN: render them as CORROBORATIVE — consistent with other evidence, incomplete, occasionally contradictory — never as a pristine, professionally-indexed laboratory record, unless the story explicitly establishes it was maintained under a formal chain-of-custody protocol, which licenses CERTAIN.
- CAUSAL CERTAINTY (real-world, historical, or investigative causal claims only — a strict or hybrid factual_regime; this does NOT apply to stipulated plot causation in a pure fictional regime, where an established story fact needs no evidentiary hedging — see the FACTUAL RENDERING block below for which regime applies here): a claim that one act caused a specific downstream harm (an illness, an injury, a death) in a NAMED individual is PROBABLE at most unless the story gives a specific, on-page mechanism connecting cause to that individual — hedge it the same way the FACTUAL RENDERING block below already asks for contested mechanisms (attribution, "likely", or downgrade to observed correlation), not with a new, competing phrasing.
- NECESSITY / PARITY CLAIMS ABOUT CORROBORATIVE EVIDENCE (real-world/investigative evidentiary-weight claims only — a strict or hybrid factual_regime; this does NOT apply to stipulated fictional-regime plot causation or ordinary persuasion/decision narrative, where a character weighing what convinced them needs no evidentiary hedging — see the FACTUAL RENDERING block below for which regime applies here; and, within that regime, only once the manuscript itself has drawn a corroborative-vs-dispositive distinction for a piece of evidence — dormant otherwise): once evidence is established as corroborative (a lead, not what "convicts" or proves a case on its own), no later passage may claim it was NECESSARY to the outcome alongside the dispositive evidence — "wouldn't have held together without it", "wasn't enough on its own", "couldn't have won/convicted without it", "wouldn't have been enough alone", or similar necessity/parity phrasing applied to a piece of evidence already established elsewhere as corroborative-only — even when it avoids overtly absolute language: a necessity claim is the same CERTAIN-tier overclaim in different phrasing, so downgrade to PROBABLE ("helped", "supported", "was consistent with") or cut. This holds for EVERY scene in the chapter, including the chapter's closing scene, and for a scene written in a different POV than the one that first established the corroborative/dispositive split — a scene break or POV change never resets a distinction the chapter already established.
- ASSERTED-AUTHENTICATION CLAIMS (real-world/investigative document- or exhibit-authentication claims only — a strict or hybrid factual_regime; this does NOT apply to a stipulated fictional-regime prop whose genuineness is simply a story fact, which needs no shown verification method — see the FACTUAL RENDERING block below for which regime applies here; and, within that regime, only when the manuscript states a document's, signature's, or physical exhibit's authentication or verification as an accomplished fact — dormant otherwise): if a document, signature, or exhibit is described as "authenticated", "verified genuine", "confirmed real", or similar, the manuscript must show the verification method on-page at least once for that exhibit — an expert's comparison testimony, a described forensic/lab process, or an on-page chain-of-custody account — rather than asserting the conclusion alone; a bare assertion with no shown method reads as invented authority and downgrades to PROBABLE ("believed genuine", "treated as authentic") rather than CERTAIN ("authenticated", "verified"). This matters most when a comparable exhibit elsewhere in the SAME manuscript DOES get its authentication process shown on-page: leaving one key document less-verified than a lesser one reads as an unintentional gap, not a deliberate authorial choice.
"""
