...
import { getConfig, setConfig, getTtsProfiles, saveTtsProfiles, deleteTtsProfile, resolveTenantId, resolveUserId, _uuid5, getOrCreateSession, appendMessage, logUsage, calcGoogleCost } from "./db.js";
...
const BATCH_JOBS_FILE  = path.join(DATA_DIR, "jobs.json");
const TTS_JOBS_FILE    = path.join(DATA_DIR, "tts-jobs.json");
const IMAGEN_JOBS_FILE = path.join(DATA_DIR, "imagen-jobs.json");
const CONFIG_FILE      = path.join(DATA_DIR, "config.json");
const TTS_PROFILES_FILE = path.join(DATA_DIR, "tts-profiles.json");
...
const mkJobId   = ()      => `job_${Date.now()}_${Math.random().toString(36).slice(2,7)}`;
...
// ══════════════════════════════════════════════════════════════════════════════
app.get("/api/jobs", (_,res)=>res.json(readJson(BATCH_JOBS_FILE,[])));
app.post("/api/submit", requireAuth, async(req,res)=>{
  ...
  try{
    ...
    const all=readJson(BATCH_JOBS_FILE,[]);all.unshift(record);writeJson(BATCH_JOBS_FILE,all);
    try{fs.unlinkSync(jsonlPath);}catch{}
    res.json({ok:true,jobName:batch.name,record});
...
app.get("/api/status", async(req,res)=>{
  ...
  try{
    ...
    const all=readJson(BATCH_JOBS_FILE,[]),idx=all.findIndex(j=>j.jobName===name);
    if(idx>=0){all[idx].state=state;writeJson(BATCH_JOBS_FILE,all);}
...
app.post("/api/retrieve", async(req,res)=>{
  ...
  try{
    ...
    const all=readJson(BATCH_JOBS_FILE,[]),record=all.find(j=>j.jobName===jobName);
    ...
    if(record){record.state=state;record.retrieved=saved;writeJson(BATCH_JOBS_FILE,all);}
...
async function runLaozhangTtsJob(jobId,{apiKeys,model,voice,speed,language,audiobookMode,silenceSeconds,audioProfile,transcriptBody,outputPrefix}){
  ...
  for(let i=0;i<lines.length;){
    if(ttsCancelFlags.get(jobId)){
      ...
      const all=readJson(TTS_JOBS_FILE,[]),idx=all.findIndex(j=>j.jobId===jobId);
      if(idx>=0){all[idx].status="cancelled";all[idx].files=job.files;writeJson(TTS_JOBS_FILE,all);}
      return;
    }
    ...
    try{
      ...
      if(!resp.ok){
        ...
        else{log(`🔴 ${resp.status}: ${errTxt.slice(0,120)}`);await new Promise(r=>setTimeout(r,3000));i++;job.progress=i;continue;}
      }
  ...
  const all=readJson(TTS_JOBS_FILE,[]),idx=all.findIndex(j=>j.jobId===jobId);
  if(idx>=0){all[idx].status="done";all[idx].files=job.files;writeJson(TTS_JOBS_FILE,all);}
}

async function runTtsJob(jobId,{apiKeys,model,voice,silenceSeconds,audioProfile,transcriptBody,outputPrefix}){
  const job=activeJobs.get(jobId);
  ...
  for(let i=0;i<lines.length;){
    ...
    try{
      ...
      await new Promise(r=>setTimeout(r,1000));i++;job.progress=i;
    }catch(e){
      const msg=String(e);
      ...
      else{log(`🔴 ${msg.slice(0,80)}`);await new Promise(r=>setTimeout(r,5000));i++;job.progress=i;}
    }
  }
  job.status="done";log("✅ Complete");
  const all=readJson(TTS_JOBS_FILE,[]),idx=all.findIndex(j=>j.jobId===jobId);
...
app.get("/api/tts/jobs",  (_,res)=>res.json(readJson(TTS_JOBS_FILE,[])));
...
app.get("/api/tts/job/:id",(req,res)=>{
  const j=activeJobs.get(req.params.id);if(j)return res.json(j);
  const s=readJson(TTS_JOBS_FILE,[]).find(x=>x.jobId===req.params.id);
  if(s)return res.json({status:"done",progress:s.total||0,total:s.total||0,files:s.files||[],logs:[]});
  res.status(404).json({error:"Not found"});
});
app.post("/api/tts/start", requireAuth, (req,res)=>{
  ...
  const lines=(apiMode==="laozhang"&&audiobookMode)
    ? chunkTextForAudiobook(transcriptBody.trim(),4000)
    : transcriptBody.trim().split(/\n\n+/).filter(l=>l.trim());
  const record={jobId,apiMode,model,voice,outputPrefix,status:"queued",total:lines.length,createdAt:new Date().toISOString(),files:[]};
  activeJobs.set(jobId,{...record,progress:0,logs:[],files:[]});
  const all=readJson(TTS_JOBS_FILE,[]);all.unshift(record);writeJson(TTS_JOBS_FILE,all);
  const runner=apiMode==="laozhang"?runLaozhangTtsJob:runTtsJob;
  runner(jobId,{apiKeys:keys,model,voice,speed,language,audiobookMode,silenceSeconds,audioProfile,transcriptBody,outputPrefix}).catch(e=>{const j=activeJobs.get(jobId);if(j){j.status="error";j.logs.push("Fatal: "+e.message);}});
  res.json({ok:true,jobId,total:lines.length});
});
app.post("/api/tts/cancel/:id",(req,res)=>{
...
async function runImagenJob(jobId,{apiKey,model,prompts,outputPrefix,aspectRatio,resolution}){
  const job=activeJobs.get(jobId);job.total=prompts.length;job.progress=0;job.status="running";
  ...
  job.status="done";log("✅ Complete");
  const all=readJson(IMAGEN_JOBS_FILE,[]),idx=all.findIndex(j=>j.jobId===jobId);
  if(idx>=0){all[idx].status="done";all[idx].files=job.files;writeJson(IMAGEN_JOBS_FILE,all);}
}
app.get("/api/imagen/jobs",  (_,res)=>res.json(readJson(IMAGEN_JOBS_FILE,[])));
...
app.get("/api/imagen/job/:id",(req,res)=>{
  const j=activeJobs.get(req.params.id);if(j)return res.json(j);
  const s=readJson(IMAGEN_JOBS_FILE,[]).find(x=>x.jobId===req.params.id);
  if(s)return res.json({status:"done",progress:s.total||0,total:s.total||0,files:s.files||[],logs:[]});
  res.status(404).json({error:"Not found"});
});
app.post("/api/imagen/start", requireAuth, (req,res)=>{
  ...
  const jobId=mkJobId();
  const record={jobId,model,outputPrefix,status:"queued",total:prompts.length,createdAt:new Date().toISOString(),files:[]};
  activeJobs.set(jobId,{...record,progress:0,logs:[],files:[]});
  const all=readJson(IMAGEN_JOBS_FILE,[]);all.unshift(record);writeJson(IMAGEN_JOBS_FILE,all);
  runImagenJob(jobId,{apiKey:key,model,prompts,outputPrefix,aspectRatio,resolution}).catch(e=>{const j=activeJobs.get(jobId);if(j){j.status="error";j.logs.push("Fatal:"+e.message);}});
