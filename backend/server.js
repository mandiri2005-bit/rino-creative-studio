...
app.post("/api/generate-image/google", async (req,res) => {
  try {
    const { prompt, model="imagegeneration@006", fallback_model, aspect_ratio="16:9", image_size="1K", nusantara_corpus=false, ref_image_b64, ref_image_mime } = req.body || {};
    ...
    // ── Primary: Google Vertex (Nano Banana family) ──
    ...
    const data = await r.json().catch(()=>({ error:"vertex returned non-JSON" }));
    // ── Failover: Vertex quota/error → LaoZhang (item 4). Vertex debits only AFTER
    // success, so a failed primary charged nothing — the LaoZhang retry bills once. ──
    if ((!r.ok || data?.error) && fallback_model && ![402,403,429].includes(r.status)) {  // gate rejections (credits/tier/free-pool) pass through, no wasteful retry
      console.warn(`[generate-image/google] vertex failed (${r.status} ${data?.error||""}) → failover LaoZhang model=${fallback_model}`);
      const r2 = await fetch(`${PYTHON_API}/generate-image`, {
        method:"POST", headers:_h,
        body: JSON.stringify({ prompt, model: fallback_model, aspect_ratio, image_size }),
      });
      const data2 = await r2.json().catch(()=>({ error:"laozhang returned non-JSON" }));
      if (r2.ok && !data2?.error) { data2._failover = "laozhang"; return res.status(200).json(data2); }
      return res.status(r2.status||r.status||502).json({ error: data2?.error || data?.error || "image gen failed (vertex + laozhang)" });
    }
