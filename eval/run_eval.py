...
# eval/ lives at the REPO ROOT (moved out of data/ to escape .gitignore), while
# the live pipeline modules live in data/moat/gutenberg/. Resolve that dir and
# put it on sys.path so the live modules import cleanly. Override with the
# env var RINO_PIPELINE_DIR if the layout ever changes.
configure_model = COLLECTION = None
get_narration_context = compare_rag_vs_standard = get_style_config = None

PIPELINE_DIR = Path(
    os.environ.get("RINO_PIPELINE_DIR", HERE.parent / "data" / "moat" / "gutenberg")
)
...
def _import_pipeline() -> None:
    ...
    if not PIPELINE_DIR.is_dir():
        print(f"[run_eval] FATAL: pipeline dir not found: {PIPELINE_DIR}")
        print("  Set RINO_PIPELINE_DIR to the folder holding rag_narration.py.")
        raise SystemExit(1)
    ...
    try:
        ...
        print(f"  Looked in: {PIPELINE_DIR}")
        print("  Need rag_narration.py, qdrant_index_v2.py, style_rag_config.py there,")
        print("  the project's venv active, and Qdrant reachable.")
        raise
...
def load_golden(path: Path) -> list[dict]:
    ...
    # golden_set.yaml is a top-level list; tolerate {entries: [...]} too.
    entries = data["entries"] if isinstance(data, dict) and "entries" in data else data
    if not isinstance(entries, list) or not entries:
        raise SystemExit(f"[run_eval] no entries found in {path} (expected a list)")
    seen = set()
    for e in entries:
